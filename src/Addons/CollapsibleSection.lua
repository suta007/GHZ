...
-- Features: Auto-Layout, Lua 5.1 Support, Snapshot Capture, Visibility Toggle, Searchable Dropdown
return function(FluentLibrary)
	...
	local function CreateCollapsibleSection(self, Title, Opened)
		...
		function Section:AddSearchableDropdown(ID, Config)
			...
			-- 1. สร้างช่อง Input ค้นหา (มันจะถูกดึงเข้า Collapsible อัตโนมัติเพราะใช้ Section:AddInput)
			Section:AddInput(ID .. "_Search", {
				Title = "🔍 Search: " .. (Config.Title or "Items"),
				Default = "",
				...
				Callback = function(searchText)
					...
					if searchText == "" then
						...
						for _, item in ipairs(originalValues) do
							if string.find(string.lower(item), string.lower(searchText)) then table.insert(filteredValues, item) end
						end
					end

					-- อัปเดต Dropdown โดยเรียกผ่าน FluentLibrary.Options
					if FluentLibrary.Options[ID] then FluentLibrary.Options[ID]:SetValues(filteredValues) end
				end,
			})

			-- 2. สร้าง Dropdown ตัวหลัก
			return Section:AddDropdown(ID, Config)
		end
